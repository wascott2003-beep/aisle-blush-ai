/**
 * Analyzes an image file for blur and darkness using canvas.
 * Returns a flag reason if the image is low quality, or null if it's fine.
 */
export async function analyzeImageQuality(file: File): Promise<'low_quality_photo' | null> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      resolve((isDark || isBlurry) ? 'low_quality_photo' : null);
    };
